...
def main():
    """
    Lists PG connections that are "idle in transaction", meaning stuck in a
    transaction that was neither committed nor rolled backed. See bug #6607 for what
    this script is trying to detect.
    """

    for pg_pid, client_addr, client_port, query in list_pg_connections():
        pids = subprocess.check_output(['lsof', '-ti', f'tcp@{client_addr}:{client_port}'], text=True).strip()
        client_pid = next(pid for pid in pids.split('\n') if pid != pg_pid)
        client_command = subprocess.check_output(['ps', '-p', str(client_pid), '-o', 'cmd', 'h'], text=True).strip()
